...
SPDP $\approx$ 25\%) reported in the paper, run on the paper's instance
sizes $n \in \{6, 8, 10\}$.
\end{enumerate}
...
\begin{table}[h]
\centering
\caption{Verification suite results. Values for V5/V6 are produced by
re-running \texttt{verification.py} with the academic licence; placeholders
below are to be replaced by the printed solver output.}
...
V5 & no sub-tours, $n{=}6$             & TBD     & TBD   & passed (single path) \\
V6 & DAPDP $\le$ PDP, $n{=}8$          & TBD     & TBD   & passed \\
...
\begin{table}[h]
\centering
\caption{Mean SPDP and runtime per $(n, \text{grid})$ cell. Re-run
\texttt{validation.py} after solver completion to populate.}
