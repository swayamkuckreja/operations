...
Table A.5. With a full Gurobi licence we can run the same instance sizes
the paper reports: n in {6, 8, 10} on grids {10, 20, 30}.

The paper does not publish per-seed instance data, so we cannot match
individual rows; instead we report mean SPDP and mean runtime over a
small ensemble of seeds and verify that they fall in the same ballpark
as the paper's reported means (SPDP ~ 25%, runtime seconds-to-minutes
for these sizes).
"""
...
def main():
    rows = []
    # Number of seeds per cell — fewer at larger n to keep total runtime bounded.
    seeds_per_n = {6: 5, 8: 3, 10: 2}
    # Time limit per solve scales with n (paper reports up to a few minutes
    # at n = 10).
    ...
    for n in [6]:
        ...
        for grid in [10.0, 20.0, 30.0]:
            ...
            for seed in range(1, n_seeds + 1):
                ...
                t0 = time.time()
                sol = solve_dapdp(inst, time_limit=tlim)
                t_dapdp = time.time() - t0
                sol_pdp = solve_truck_only(inst, time_limit=tlim)
                ...
                print(f"  n={n} grid={grid:4.0f} seed={seed}: "
                      f"PDP=${sol_pdp.objective:7.3f}  DAPDP=${sol.objective:7.3f}  "
