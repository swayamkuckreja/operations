...
def scenario_no_subtours() -> None:
    ...
    inst = generate_instance(n=4, grid_size=15.0, seed=11)
    sol = solve_dapdp(inst, time_limit=180.0)
    a = audit(inst, sol)
    print(a)
...
def scenario_pdp_vs_dapdp() -> None:
    ...
    inst = generate_instance(n=4, grid_size=10.0, seed=3)
    sol_pdp = solve_truck_only(inst, time_limit=120.0)
    sol_dapdp = solve_dapdp(inst, time_limit=120.0)
