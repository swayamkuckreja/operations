...
Instance budget. The free Gurobi license (~2000 vars/constraints) caps us
at n <= 4. We therefore use a small ensemble of n=3 and n=4 instances
across grid sizes {5, 10, 20}. This is small but enough to show clean
trends consistent with the paper.
"""
...
def _make_ensemble() -> List[Instance]:
    """A small but balanced ensemble of test instances (within free-license
    size limits)."""
    out: List[Instance] = []
    for n in [3, 4]:
...
def sweep_endurance(instances: List[Instance],
                    ...
                    time_limit: float = 60.0) -> List[Run]:
...
def main():
    ...
    print(f"Sensitivity ensemble: {len(instances)} instances "
          f"(n in {{3,4}}, grid in {{5,10,20}}, seeds {{1,2,3}})")

    end_rows = sweep_endurance(instances)
    spd_rows = sweep_drone_speed(instances)
    alp_rows = sweep_alpha(instances)
